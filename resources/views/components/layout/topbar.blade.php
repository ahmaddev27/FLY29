...
<header class="sticky top-0 z-20 flex items-center justify-between gap-2 sm:gap-4 px-3 sm:px-6 py-3 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
    ...
    <div class="flex items-center gap-1 sm:gap-3 flex-shrink-0">
        ...
        {{-- Notifications dropdown — listens to Firebase via window event --}}
        <div
            x-data="{
                open: false,
                items: [],
                unread: 0,
                init() {
                    window.addEventListener('firebase-notifications', (e) => {
                        this.items  = e.detail.items;
                        this.unread = e.detail.unreadCount;
                    });
                },
                markOne(id)   { window.firebaseNotifications?.mark(id); },
                markAll()     { window.firebaseNotifications?.markAll(this.items); },
            }"
            class="relative"
        >
            <button
            ...
            >
                ...
                <span
                    x-show="unread > 0"
                    x-cloak
                    class="absolute top-1 end-1 min-w-[18px] h-[18px] px-1 rounded-full bg-[var(--color-danger-500)] text-white text-[10px] font-bold flex items-center justify-center"
                ...
                <div class="max-h-96 overflow-y-auto">
                    ...
                    <template x-for="item in items" :key="item.id">
                        <a
                            ...
                            x-on:click="markOne(item.id)"
                            class="block px-4 py-3 border-b border-[var(--color-surface-divider)] hover:bg-[var(--color-surface-tertiary)] transition-base"
                            :class="!item.is_read ? 'bg-[var(--color-primary-50)]/30' : ''"
                        >
                            <div class="flex items-start gap-2">
                                <span
                                    x-show="!item.is_read"
                                    class="w-2 h-2 rounded-full bg-[var(--color-primary-500)] mt-1.5 flex-shrink-0"
                                ></span>
                                <div class="flex-1 min-w-0">
                                    <p class="font-medium text-sm text-[var(--color-text-primary)]" x-text="item.title"></p>
                                    <p class="text-xs text-[var(--color-text-secondary)] mt-0.5" x-text="item.body"></p>
                                </div>
                            </div>
                        </a>
                    </template>
